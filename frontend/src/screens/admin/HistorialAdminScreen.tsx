...
const HistorialAdminScreen: React.FC = () => {
      ...
      const fetchAsistencias = async () => {
        try {
          ...
          const data: Asistencia[] = querySnapshot.docs.map((doc) => {
            ...
            const fechaAsistencia = docData.fecha instanceof Timestamp
                ? docData.fecha.toDate()
                : new Date();
            return {
              id: doc.id,
              voluntarioNombre: docData.voluntarioNombre || 'Nombre no disponible',
          ...
          const sections: AsistenciaSection[] = Object.keys(grouped).map((key) => ({
            title: key, data: grouped[key],
          }));
    ...
    <View style={styles.card}>
      ...
      <View style={styles.infoContainer}>
        ...
        <Text style={styles.itemDetails}>
          {`ID: ${item.voluntarioId.substring(0, 8)}...`}
          {'\n'}
          {item.fecha.toLocaleDateString('es-ES', { day: 'numeric', month: 'long' })}
          {'\n'}
          Entrada: {item.fecha.toLocaleTimeString('es-ES', {
  ...
  const renderSectionHeader = ({ section: { title } }: { section: AsistenciaSection }) => (
    ...
    if (error) {
      return (
        <View style={styles.loadingContainer}>
          ...
          <Text style={{ marginTop: 15, fontSize: 16, color: Colors.gray, textAlign: 'center' }}>
            {error}
          </Text>
        </View>
      );
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: Colors.background}}>
      <View style={styles.container}>
        <HeaderBack title="Historial de asistencias" onBack={() => navigation.goBack()} />
        {renderContent()}
      </View>
    </SafeAreaView>
  );
